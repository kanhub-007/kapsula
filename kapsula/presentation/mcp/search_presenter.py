...
def format_search_results(
    ...
    for i, result in enumerate(results, 1):
        ...
        # Signal indicator
        has_dense = dense is not None and dense > 0.15
        has_sparse = sparse is not None and sparse > 0
        ...
        out.append(
            f"--- Result {i} [{src}/{doc}]{sub_info} ---"
        )
        out.append(
            f"  score: {score_line} ({signal}) | context: {ctx_label}"
        )
