...
class HuggingFaceChatClient:
    ...
    def __init__(self, token: str, model: str, timeout: int | None = None):
        self._client = InferenceClient(token=token, timeout=timeout or DEFAULT_LLM_TIMEOUT)
        self._model = model
    ...
    def send(
        ...
        try:
            ...
            logger.error(f"HuggingFace API error (status {getattr(e, 'response', {}).get('status_code', '?')}): {e}")
            raise
