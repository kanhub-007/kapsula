...
def extract_subdocuments(markdown_content: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for line in lines:
        ...
        if h1_match:
            ...
            page_title = breadcrumb_data["levels"][-1] if breadcrumb_data["levels"] else breadcrumb_data["domain"]
...
def validate_subdocuments(subdocuments: Dict[str, List[Dict[str, Any]]]) -> bool:
    ...
    logger.info(f"Validation: {len(subdocuments)} sub-documents, {total_pages} total pages")
    return True
